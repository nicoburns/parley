...
fn first_line_items(layout: &Layout<ColorBrush>) -> Vec<PositionedLayoutItem<'_, ColorBrush>> {
    layout.lines().next().unwrap().items().collect::<Vec<_>>()
}
...
fn vertical_align_baseline_is_default() {
    ...
    let layout_explicit =
        build_layout_with_valign_text(&mut env, &[("Hello world", DEFAULT.0, DEFAULT.1)]);
...
fn vertical_align_text_sub_lowers_baseline() {
    ...
    let normal_baseline = baselines
        .iter()
        .find(|(t, _)| t.starts_with('N'))
        .unwrap()
        .1;
    let sub_baseline = baselines
        .iter()
        .find(|(t, _)| t.starts_with('s'))
        .unwrap()
        .1;
...
fn vertical_align_text_length_positive_raises() {
    ...
    let normal_baseline = baselines
        ...
        .unwrap()
        .1;
    let raised_baseline = baselines
        .iter()
        .find(|(t, _)| t.starts_with('r'))
        .unwrap()
        .1;
...
fn vertical_align_text_length_negative_lowers() {
    ...
    let normal_baseline = baselines
        ...
        .unwrap()
        .1;
    let lowered_baseline = baselines
        .iter()
        .find(|(t, _)| t.starts_with('l'))
        .unwrap()
        .1;
...
fn vertical_align_large_length_expands_line_visual_extent() {
    ...
    let layout_normal = build_layout_with_valign_text(&mut env, &[("Hello", DEFAULT.0, DEFAULT.1)]);
    let metrics_normal = *layout_normal.lines().next().unwrap().metrics();
    let extent_normal = metrics_normal.block_max_coord - metrics_normal.block_min_coord;
    ...
    let layout_raised = build_layout_with_valign_text(
        ...
        &[
            ...
            (
                "UP",
                AlignmentBaseline::Baseline,
                BaselineShift::Length(50.0),
            ),
        ],
    );
    let metrics_raised = *layout_raised.lines().next().unwrap().metrics();
    let extent_raised = metrics_raised.block_max_coord - metrics_raised.block_min_coord;
...
fn vertical_align_inline_box_middle() {
    ...
    let x_height = line
        .runs()
        .next()
        .map(|r| r.metrics().x_height.unwrap_or(r.font_size() * 0.5))
        .unwrap_or(0.0);
...
fn vertical_align_combined_text_top_with_shift() {
    ...
    let normal_baseline = baselines
        ...
        .unwrap()
        .1;
    let shifted_baseline = baselines
        .iter()
        .find(|(t, _)| t.starts_with('s'))
        .unwrap()
        .1;
